...
export class CoordinatorAgent {
  ...
  async executeTask(userPrompt) {
    ...
    try {
        ...
        console.error("Agent execution failed:", e?.status, e?.message);
        
        // Handle leaked key or authentication errors with a Demo Mode fallback
        if (e?.message?.includes('leaked') || e?.message?.includes('API key') || e?.status === 403) {
            console.log("[Coordinator] Switching to Demo Mode fallback.");
            if (userPrompt.toLowerCase().includes("hi") || userPrompt.toLowerCase().includes("hello")) {
                return "👋 Hello! I'm currently running in **Demo Mode** because the provided Gemini API key is invalid or has been leaked. I can still help you explore the UI, but my full AI scheduling powers are temporarily paused. To enable them, please update the `GEMINI_API_KEY` in your `.env` file!";
            }
            if (userPrompt.toLowerCase().includes("task") || userPrompt.toLowerCase().includes("schedule")) {
                return "📅 In Demo Mode, I can't live-calculate your schedule, but I've already optimized your dashboard based on the 'Rainy Day' toggle! Try clicking the 'Rainy Day/Normal Day' button at the top to see me in action.";
            }
            return "🤖 (Demo Mode): I received your message: \"" + userPrompt + "\". Unfortunately, I can't process complex AI tasks without a valid API key. However, the rest of the application is fully functional!";
        }
